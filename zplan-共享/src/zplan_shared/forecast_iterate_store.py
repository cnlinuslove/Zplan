...
INDEX_NAME = "forecast_iterations.jsonl"
...
def append_forecast_iteration(record: dict[str, Any]) -> Path:
    """追加索引并写入当轮快照。"""
    iid = str(record["iteration_id"])
    snap = iteration_dir() / f"forecast_{iid}.json"
    snap.write_text(
...
def list_forecast_iterations(*, limit: int = 20) -> list[dict[str, Any]]:
    idx = _index_path()
    if not idx.is_file():
        return []
...
def load_forecast_iteration(iteration_id: str) -> dict[str, Any] | None:
    p = iteration_dir() / f"forecast_{iteration_id}.json"
    if not p.is_file():
        return None
    return json.loads(p.read_text(encoding="utf-8"))


def compare_forecast_iterations(
    ...
    keys = ("accuracy_pct", "verified_count", "mean_confidence")
    deltas: dict[str, Any] = {}
    for k in keys:
    ...
    return {
        ...
        "deltas": deltas,
        "improved": (
            (deltas.get("accuracy_pct") is not None and deltas["accuracy_pct"] > 0)
        ),
    }
