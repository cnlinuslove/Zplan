...
def enrich_single(ts_code: str, as_of: str = "2026-06-05", fast: bool = False) -> dict[str, bool]:
    ...
    for attempt in range(3):
        try:
            ...
            status = {"profile": False, "peers": False, "reports": False, "holdings": False}
...
def main():
    ...
    if len(sys.argv) < 2:
        print("用法: --symbol 300124 | --batch [--fast] [--p0]")
        sys.exit(1)
    ...
    if "--symbol" in sys.argv:
        ...
        success = {"profile": 0, "peers": 0, "reports": 0, "holdings": 0}
        done = 0

        def _worker_one(ts):
            try:
                return enrich_single(ts, fast=fast)
            except Exception as e:
